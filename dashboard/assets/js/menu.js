...
jq141(function($) {
	...
	$('#form_menu').bind('submit', function(e) {
		var $this = $(this);
		$('#btn-save-menu').attr('disabled', true);
		$.ajax({
			type: 'POST',
			...
			success: function(data) {
                ...
                setTimeout(function(){
                    $(".notification-msg").addClass('hide').html('');
                }, 2000);
				$this.find('button[type=submit]').html('<i class="fa fa-floppy-o" style="margin-right:5px;"></i> Simpan Perubahan');
			},
			beforeSend: function(){
                $(".notification-msg").removeClass('hide').html('');;
				$this.find('button[type=submit]').html('<i class="fa fa-floppy-o" style="margin-right:5px;"></i> Memproses Penyimpanan...');
			}
		});
