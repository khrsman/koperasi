...
class Alamat_mod extends CI_Model {
	...
	function get_nama_provinsi_koperasi(){

		$koperasi = $this->session->userdata('koperasi');
		$this->db->select("ref_provinsi.nama, (SELECT COUNT(user_alamat.id_user) FROM user_alamat JOIN user_info ON user_info.id_user = user_alamat.id_user WHERE pengirim_provinsi = id_provinsi AND user_info.koperasi  = '$koperasi') as jumlah");
		$this->db->from('ref_provinsi');
		$this->db->order_by('id_provinsi', 'asc');
		return $this->db->get();
	}
