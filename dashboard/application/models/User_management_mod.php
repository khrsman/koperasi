...
class User_management_mod extends CI_Model {

	function get_all_admin($keyword=NULL,$limit=NULL,$offset=NULL,$param_query=NULL){
		$this->db->select('SQL_CALC_FOUND_ROWS *, (select nama from user_level where id_level = level) lvl',FALSE);
	...
	function update_basic($id){
		...
		$object2 = array('level' 		=> $this->input->post('level'));
		$this->db->where('id_user', $id);
		$this->db->update("user_info", $object2);		
	}
