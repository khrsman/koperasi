...
class Produk_mod extends CI_Model {
	...
	function get_peruntukan_produk_by_id($id_produk, $id_koperasi){
		$this->db->select('produk_admin.*, koperasi.nama as nama_koperasi, koperasi.id_koperasi, produk.id_produk as produk_id');
		$this->db->from('produk_admin');
		$this->db->join('produk', 'produk.id_produk = produk_admin.id_produk');
		$this->db->join('koperasi', 'koperasi.id_koperasi = produk_admin.user_target');
	...
	function get_peruntukan_produk($id){
		$this->db->select('produk_admin.*, koperasi.nama as nama_koperasi, koperasi.id_koperasi, produk.id_produk as produk_id');
		$this->db->from('produk_admin');
		$this->db->join('produk', 'produk.id_produk = produk_admin.id_produk');
		$this->db->join('koperasi', 'koperasi.id_koperasi = produk_admin.user_target');
	...
	function update_produk_peruntukan($id_produk, $id_koperasi){
		
		$input_baru = $this->input->post('qty');
		$stok_koperasi = $this->input->post('qty_hidden');
		$stok_admin = $this->get_stock()['qty'];

		if($input_baru <= $stok_koperasi){
			$SELISIH = $stok_koperasi - $input_baru;
			$stok_admin = $stok_admin + $SELISIH;
			$stok_koperasi = $input_baru;
		}
		else if($input_baru >= $stok_koperasi)
		{
			$SELISIH = $input_baru - $stok_koperasi;
			$stok_admin = $stok_admin - $SELISIH;
			$stok_koperasi = $stok_koperasi + $SELISIH;
		}
		...
		$produk_admin = array('price_n' 		=> $this->input->post('price_n'),
		...
		$produk_admin_history = array(	  'id_produk' 		=> $this->session->userdata('id'),
										  'price_n' 		=> $this->input->post('price_n'),
										  'price_s' 		=> $this->input->post('price_s'),
										  'qty' 			=> $stok_koperasi,
										  'user_target' 	=> $id_koperasi,
										  'service_time'	=> date('Y-m-d H:i:s'),
										  'service_action' 	=> 'update',
										  'service_user' 	=> $this->session->userdata('id_user'));
		...
		$this->db->where('id_produk', $id_produk);
		$this->db->where('user_target', $id_koperasi);
		$this->db->update('produk_admin', $produk_admin);

		$this->db->insert('produk_admin_history', $produk_admin_history);

	}

	function add_peruntukan(){
		$qty_awal = $this->get_stock()['qty'];
		$qty_akhir = $qty_awal - $this->input->post('qty');


		$produk_admin = array('id_produk' 		=> $this->session->userdata('id'),
							  'price_n' 		=> $this->input->post('price_n'),
							  'price_s' 		=> $this->input->post('price_s'),
							  'qty' 			=> $this->input->post('qty'),
							  'user_target' 	=> $this->input->post('koperasi'),
							  'service_time'	=> date('Y-m-d H:i:s'),
							  'service_action' 	=> 'insert',
							  'service_user' 	=> $this->session->userdata('id_user'));

		$produk = array('qty' 					=> $qty_akhir,
						'service_time'			=> date('Y-m-d H:i:s'),
					  	'service_action'		=> 'insert',
					  	'service_user'			=> $this->session->userdata('id_user'));
		...
		$this->db->insert('produk_admin', $produk_admin);
		$this->db->insert('produk_admin_history', $produk_admin_history);
