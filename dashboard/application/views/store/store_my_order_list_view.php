	...
	<div class="col-md-12">
        ...
        <div class="box-header">
          ...
          <div class="row">
            ...
            <div class="col-md-7">
            <h4><i class="fa fa-cart-plus" style="margin-right:7px"></i> Daftar Pesanan Saya</h4>
            </div>

            <div class="col-md-7 hidden">
              <h4>Status : 
                <?php foreach($filter_koperasi as $k => $v): ?>
                  <a href="<?php echo site_url('store_my_order/l').update_query_string('filter_store_order_status',$v['alias']); ?>" 
            ...
            <div class="col-md-5 pull-right">
              <?php  
                ...
                echo form_open(site_url('store_my_order/l'),$attributes);
              ?>
            ...
            <div class="col-md-3">
              ...
              <select class="text-muted select-redirect">
                <?php foreach($sort as $k => $v): ?>
                  <option value="<?php echo site_url('store_my_order/l').update_query_string('sort',$v['alias']); ?>" <?php if($v['alias']==$param_sort){echo 'selected';} ?>><?php echo $v['alias']; ?></option>
                <?php endforeach; ?>  
              </select>
              <select class="text-muted select-redirect">
                <?php foreach($sort_order as $k => $v): ?>
                  <option value="<?php echo site_url('store_my_order/l').update_query_string('sort_order',$v['alias']); ?>" <?php if($v['alias']==$param_sort_order){echo 'selected';} ?>><?php echo $v['alias']; ?></option>
                <?php endforeach; ?>  
              </select>
            </div>
        ...
        <div class="box-body">
          <table id="table-1" class="table table-bordered table-consended table-hover">
            ...
            <?php foreach ($product as $k => $v):?>		
              <tr>
                <td>
                   <a class="text-primary" href="<?php echo site_url('store_my_order/detail').'/'.$v['no_transaksi']; ?>">
                ...
                <td>
                  <a class="btn btn-sm btn-info" href="<?php echo site_url('store_my_order/detail').'/'.$v['no_transaksi']; ?>">Detail</a>
                </td>
              </tr>
            <?php endforeach; ?>  
            <?php endif; ?>
