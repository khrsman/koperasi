...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Total Pembelian</li>
    </ol>
</section>
...
<section class="content">
    ...
    <div class="box box-success table-responsive no-padding">
            <div class="box-header">
              <h3 class="box-title">Total Pembelian Berdasarkan Provinsi</h3>
            </div>
            <div class="box-body">
              ...
              foreach ($transaksi as $p) : ?>
                <tr> 
                  ...
                  <td style="text-align: right;"> Rp. <?php
                                                                if($p->total != NULL):
                                                                $total = str_replace(".00", "",  $p->total);
                                                                else:
                                                                $total = 0;
                                                                endif;
                                                                echo number_format($total,0,".",",") ?></td>
                  <td style="text-align: right;"><button class="btn btn-info" onClick=location.href="<?= base_url() ?>get_kabupaten/<?= $p->id?>"><i class="fa fa-eye"></i> Lihat Kota/Kabupaten</td>
                </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
...
<script>
      $(function () {
        var areaChartData = {
          labels: [<?php foreach ($transaksi as $p) {
            echo '"'.$p->nama.'",';
        }?>],
          datasets: [
            {
              ...
              data: [<?php foreach ($transaksi as $p) {
            if($p->total == NULL){
            	$total = 0;
            }
            else{
