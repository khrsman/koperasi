      ...
      <div class="box-body">
        <div class="row">
          ...
          <div class="col-xs-12 col-sm-4 col-lg-4 ">
            <a class="btn btn-sm btn-success pull-right" href="<?= base_url() ?>add_user"><i class="fa fa-plus-circle"></i> Tambah User</a>
          </div>
        </div>
      </div>
    </div>
            ...
            <div class="row">
              <div class="col-md-3">
                ...
                <select class="text-muted select-redirect">
                  ...
                  <option value="<?php echo site_url('user_management').update_query_string('sort',$v['alias']); ?>" <?php if($v['alias']==$param_sort){echo 'selected';} ?>><?php echo $v['alias']; ?></option>
                  <?php endforeach; ?>
                </select>
                <select class="text-muted select-redirect">
                  <?php foreach($sort_order as $k => $v): ?>
                  <option value="<?php echo site_url('user_management').update_query_string('sort_order',$v['alias']); ?>" <?php if($v['alias']==$param_sort_order){echo 'selected';} ?>><?php echo $v['alias']; ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              ...
              <table id="table-1" class="table table-bordered table-consended table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><?php echo $v['lvl']; ?></td>
                    <td>
                        <a class="btn btn-success" href="<?= base_url()?>edit_user/<?= $v['id_user'] ?>"><i class="fa fa-pencil"></i></a>
                        <a href="<?= base_url() ?>delete_user/<?= $v['id_user']?>" class="btn btn-danger"onclick="return confirm('Anda Yakin Untuk Menghapus data ?');"><i class="fa fa-trash"></i></a>
                    </td>
                  </tr>
                  <?php endforeach; ?>
                  <?php endif; ?>
