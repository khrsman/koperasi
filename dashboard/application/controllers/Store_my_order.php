...
class Store_my_order extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('product_order_detail_model');
        $this->load->model('product_order_shipping_model');
    ...
    function l(){
        ...
        // Filter
        $param_query['filter_user'] = $filter_user;
        ...
        $config['base_url']             = site_url('store_order/l');
        ...
        $product = $this->product_order_model->get_all($keyword,$limit,$offset,$param_query);
        $data['product'] = NULL;
        if ($product['count']!=0) {
        ...
        $data['page']               = 'store/store_my_order_list_view';
    ...
    function detail($no_transaksi=NULL){
        ...
        $get_order = $this->product_order_model->get_by_transaksi($no_transaksi);
        if ($get_order==FALSE) {
            redirect('404');
        }
        ...
        $get_order_detail = $this->product_order_detail_model->get_by_transaksi($no_transaksi);
        ...
        $get_order_shipping = $this->product_order_shipping_model->get_by_transaksi($no_transaksi);
        ...
        // FIX NAMA ALAMAT
        $get_pengirim_kelurahan = $this->product_order_shipping_model->get_kelurahan_by_id($order_shipping['pengirim_kelurahan']);
        $order_shipping['pengirim_kelurahan'] = $get_pengirim_kelurahan['nama'];

        $get_pengirim_kecamatan = $this->product_order_shipping_model->get_kecamatan_by_id($order_shipping['pengirim_kecamatan']);
        $order_shipping['pengirim_kecamatan'] = $get_pengirim_kecamatan['nama'];

        $get_pengirim_kabupaten = $this->product_order_shipping_model->get_kabupaten_by_id($order_shipping['pengirim_kabupaten']);
        $order_shipping['pengirim_kabupaten'] = $get_pengirim_kabupaten['nama'];

        $get_pengirim_provinsi = $this->product_order_shipping_model->get_provinsi_by_id($order_shipping['pengirim_provinsi']);
        $order_shipping['pengirim_provinsi'] = $get_pengirim_provinsi['nama'];


        $get_penerima_kelurahan = $this->product_order_shipping_model->get_kelurahan_by_id($order_shipping['penerima_kelurahan']);
        $order_shipping['penerima_kelurahan'] = $get_penerima_kelurahan['nama'];

        $get_penerima_kecamatan = $this->product_order_shipping_model->get_kecamatan_by_id($order_shipping['penerima_kecamatan']);
        $order_shipping['penerima_kecamatan'] = $get_penerima_kecamatan['nama'];

        $get_penerima_kabupaten = $this->product_order_shipping_model->get_kabupaten_by_id($order_shipping['penerima_kabupaten']);
        $order_shipping['penerima_kabupaten'] = $get_penerima_kabupaten['nama'];

        $get_penerima_provinsi = $this->product_order_shipping_model->get_provinsi_by_id($order_shipping['penerima_provinsi']);
        ...
        // Hitung Terkirim
        $total_item = 0;
        $total_item_terkirim = 0;
        foreach ($order_detail as $k => $v) {
            $total_item = $total_item+1;
            if ($v['status_terkirim']=='Y') {
                $total_item_terkirim = $total_item_terkirim+1; 
            }
        }
        ...
        if ($total_item==$total_item_terkirim) {
            $data['status'] = 'Pesanan Terkirim';
        }else{
            $data['status'] = $total_item_terkirim.' dari '.$total_item.' Total Pesanan Terkirim';    
        }
        
        $data['return'] = $this->session->userdata('referred_from');
       
        $data['page'] = 'store/store_my_order_detail_view';
        $this->load->view('main_view',$data);
