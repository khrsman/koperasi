...
class Anggota extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('pendidikan_mod');
		$this->load->model('agama_mod');
        $this->load->model('polis_mod');
		$this->load->model('question_answer_mod');
	...
	function data_anggota(){
        ...
        $config['base_url']             = site_url('anggota');
	...
	function add_anggota(){
		...
		$this->form_validation->set_rules('no_anggota', 'No. Anggota', 'required|xss_clean');
		...
		$this->form_validation->set_rules('telepon2', 'Telepon 2', 'numeric|xss_clean');
		$this->form_validation->set_rules('telepon3', 'Telepon 3', 'numeric|xss_clean');
		...
		$this->form_validation->set_rules('rt', 'RT', 'required|numeric|xss_clean');
        $this->form_validation->set_rules('rw', 'RW', 'required|numeric|xss_clean');
        $this->form_validation->set_rules('mulai_polis', 'Tanggal Efektif Polis', 'xss_clean');
		$this->form_validation->set_rules('akhir_polis', 'Tanggal Berakhir Polis', 'xss_clean');
		$this->form_validation->set_rules('kel', 'Provinsi, Kota/Kabupaten, Kecamatan, Desa/Kelurahan', 'required|xss_clean');
		$this->form_validation->set_rules('kode_pos', 'Kode Pos', 'numeric|xss_clean');
		$this->form_validation->set_rules('npwp', 'NPWP', 'numeric|xss_clean');
		...
		$this->form_validation->set_rules('pin', 'PIN', 'required|numeric|xss_clean');
		$this->form_validation->set_rules('pendidikan', 'Pendidikan Terakhir', 'required|xss_clean');
		$this->form_validation->set_rules('tanggungan', 'Jumlah Tanggungan', 'numeric|xss_clean');
		$this->form_validation->set_rules('jumlah_hp', 'Jumlah HP', 'numeric|xss_clean');
		$this->form_validation->set_rules('jumlah_bank', 'Jumlah Akun Bank', 'numeric|xss_clean');
		$this->form_validation->set_rules('jumlah_cc', 'Jumlah Kartu Kredit', 'numeric|xss_clean');
		$this->form_validation->set_rules('jum_motor', 'Jumlah Sepeda Motor', 'numeric|xss_clean');
		$this->form_validation->set_rules('jum_mobil', 'Jumlah Mobil', 'numeric|xss_clean');
		$this->form_validation->set_rules('jumlah_rumah', 'Jumlah Rumah', 'numeric|xss_clean');


		if ($this->form_validation->run() == FALSE) {
			$data['question'] = $this->anggota_mod->get_question()->result();
			$data['agama'] = $this->agama_mod->get_all_agama()->result();
			$data['pendidikan'] = $this->pendidikan_mod->get_all_pendidikan()->result();
	...
	function anggota_edit(){
		if($this->anggota_mod->get_anggota_by_id($this->session->userdata('id'))->num_rows() > 0){
			...
			$data['pekerjaan'] = $this->pekerjaan_mod->get_all_pekerjaan()->result();
			$data['koperasi'] = $this->koperasi_mod->get_all_koperasi()->result();
			$data['question'] = $this->question_answer_mod->get_all_question()->result();
	...
	function update_contact(){
				...
				$this->form_validation->set_rules('telepon', 'Telepon', 'numeric|required|xss_clean');
				$this->form_validation->set_rules('telepon2', 'Telepon', 'numeric|xss_clean');
				$this->form_validation->set_rules('telepon3', 'Telepon', 'numeric|xss_clean');
    ...
    function select_polis(){
          ...
          $query = $this->polis_mod->get_all_polis($search);
