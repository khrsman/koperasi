...
class Transaksi_commerce extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('transaksi_commerce_mod');
	   ...
	   function list_transaksi()
	{
        ...
        $def_param_sort = 'transaksi.no_transaksi';
        ...
        $config['base_url']             = site_url('transaksi/commerce');
        ...
        $datadb = $this->transaksi_commerce_mod->get_transaksi($keyword,$limit,$offset,$param_query);
        $data['datadb'] = NULL;
        if ($datadb['count']!=0) {
        ...
        $this->load->view('log_transaksi/log_transaksi_commerce_view',$data);
	}

    function get_koperasi(){
      $search = strip_tags(trim($this->input->get('q')));
      $query = $this->transaksi_commerce_mod->get_id_nama($search);
    ...
    function get_detail_transaksi(){
        $this->session->set_userdata('id_transaksi', $this->uri->rsegment(3));
        redirect(base_url().'detail_transaksi','refresh');
    }

    function detail_transaksi(){
       if($this->transaksi_commerce_mod->get_detail_transaksi($this->session->userdata('id_transaksi'))->num_rows() > 0){
            $data['transaksi'] = $this->transaksi_commerce_mod->get_detail_transaksi($this->session->userdata('id_transaksi'))->row_array();
            $data['barang'] = $this->transaksi_commerce_mod->get_detail_transaksi($this->session->userdata('id_transaksi'))->result();
            $this->load->view('log_transaksi/detail_transaksi_commerce', $data);
       } 
       else{
