...
class Auth_model extends CI_Model {
    ...
    function insert_user(){
        ...
        if($this->session->userdata('id_register') == "1"){
            $level = "3";
            $koperasi = $this->input->post('instansi');
            $komunitas = NULL;
        }
        else{
            $level = "5";
            $koperasi = NULL;
            $komunitas = $this->input->post('instansi');
        }

        $id_alamat = "13".time();

        $date = new DateTime($this->input->post('tgl_lahir'));
    ...
    function get_instansi($search){
       if($this->session->userdata('id_register') == "1"){
        $this->db->select('id_koperasi as id, nama');
        $this->db->like('nama', $search);
        $this->db->where('status_active', "1");
        return $this->db->get('koperasi');
       }
       else if($this->session->userdata('id_register') == "2"){
        $this->db->select('id_komunitas as id, nama');
        $this->db->like('nama', $search);
        $this->db->where('status_active', "1");
        return $this->db->get('komunitas');
       }
