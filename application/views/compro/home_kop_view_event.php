...
<!-- end container -->
<div class="clearfix"></div>
<script type="text/x-tmpl" id="tmpl-list-event">
{% for(var x=0; res=o[x]; x++) { %}
  <li class="list-event" id="event-{%= res.id %}" data-time="{%= res.time %}">
    <div class="row no-margin">
      {% if(res.thumbnail) { %}
      <div class="col-xs-12 col-md-4 col-lg-3 no-padding">
        <div class="list-thumbnail" style="background-image: url(<?php echo base_url('dashboard/assets/images/event') ; ?>/{%= res.thumbnail %})"></div>
      </div>
      <div class="col-xs-12 col-md-8 col-lg-9 no-padding">
      {% } else { %}
      ...
      {% } %}
        <div class="list-content">
          <div class="list-title"><a href="<?php echo base_url('koperasi/'.$koperasi->id_koperasi.'/'.convert_to_url($koperasi->nama).'/event') ?>{%= "/" + res.id + "/" + convert_to_url(res.title_raw) %}">{%# res.title %}</a></div>
          <div class="list-time">{%= res.time_caption %}</div>
          <div class="list-desc">{%# res.content %}</div>
          <div class="list-more"><a href="<?php echo base_url('koperasi/'.$koperasi->id_koperasi.'/'.convert_to_url($koperasi->nama).'/event') ?>{%= "/" + res.id + "/" + convert_to_url(res.title_raw) %}">Baca Selengkapnya</a></div>
        </div>
      </div>
    </div>
  </li>
...
<script>
  $(document).ready(function(){
    $(".load-more > button").unbind().bind('click', function(){
      var last_time = $("#list_event").find('li:last-child').attr('data-time');
      load_event(last_time);
    });
    var load_event = function(last){
      var json_data = {
        id_koperasi: "<?php echo $koperasi->id_koperasi ?>",
        last: last ? last : null 
      };
      $.ajax({
        url: "<?php echo base_url() ?>home/ajax_get_event",
        data: json_data,
        dataType: 'JSON',
        type: 'POST',
        success: function(data){
          ...
          if(data.data.length > 0){
            $("#list_event").append(tmpl('tmpl-list-event', data.data));
          } else {
            $(".load-more").addClass('hide');
            if($("#list_event").find('li.list-event').length == 0){
              $("#list_event").html(tmpl('tmpl-list-empty'));
            }
          }
